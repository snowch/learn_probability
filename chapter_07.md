...
# Subtitle
ax.text(0.5, 0.915, r"Example: $r=3$ successes, $k=5$ total trials, $p=0.4$",
        transform=ax.transAxes, ha="center", va="top",
        fontsize=20, style="italic")

# Parameters note
param_box = mpatches.FancyBboxPatch((0.06, 0.84), 0.88, 0.045,
                                    boxstyle="round,pad=0.01",
                                    linewidth=2, edgecolor='purple',
                                    facecolor='lavender', zorder=1)
ax.add_patch(param_box)
ax.text(0.5, 0.8625, "Need exactly 2 successes in first 4 trials, then success on 5th trial",
        transform=ax.transAxes, ha="center", va="center",
        fontsize=17, weight="bold", color="purple")
...
for i, seq in enumerate(sequences):
    ...
    ax.text(x_pos + box_width/2, y_pos + box_height/2, seq,
            transform=ax.transAxes, ha="center", va="center",
            fontsize=19, weight="bold", family='monospace', zorder=3)

    # Store box coordinates for arrows (center_x, bottom_y)
    seq_boxes[seq] = (x_pos + box_width/2, y_pos)

# Counting explanation
count_box = mpatches.FancyBboxPatch((0.10, 0.56), 0.80, 0.062,
                                    boxstyle="round,pad=0.01",
                                    linewidth=2.5, edgecolor='darkorange',
                                    facecolor='lightyellow', zorder=1)
ax.add_patch(count_box)
ax.text(0.5, 0.591, r"6 ways to arrange 2 successes in first 4 trials = $\binom{4}{2} = \binom{k-1}{r-1} = 6$",
        transform=ax.transAxes, ha="center", va="center",
        fontsize=18, weight="bold", color='darkorange', zorder=4)

# Probability calculation
prob_box = mpatches.FancyBboxPatch((0.08, 0.42), 0.84, 0.115,
                                   boxstyle="round,pad=0.012",
                                   linewidth=2.5, edgecolor='darkgreen',
                                   facecolor='lightgreen', zorder=1)
ax.add_patch(prob_box)

ax.text(0.5, 0.515, "Each sequence has the same probability:",
        transform=ax.transAxes, ha="center", va="top",
        fontsize=18, weight="bold", color='darkgreen', zorder=4)

ax.text(0.5, 0.473, r"First 4 trials: $(0.4)^2 \times (0.6)^2 = 0.0576$",
        transform=ax.transAxes, ha="center", va="center",
        fontsize=17, color='darkgreen', zorder=4)

ax.text(0.5, 0.435, r"5th trial (must succeed): $0.4$",
        transform=ax.transAxes, ha="center", va="center",
        fontsize=17, color='darkgreen', zorder=4)

# Final result
result_box = mpatches.FancyBboxPatch((0.10, 0.29), 0.80, 0.10,
                                     boxstyle="round,pad=0.012",
                                     linewidth=3, edgecolor='red',
                                     facecolor='mistyrose', zorder=1)
ax.add_patch(result_box)

ax.text(0.5, 0.365, r"$P(X=5) = 6 \times 0.0576 \times 0.4 = 0.1382$",
        transform=ax.transAxes, ha="center", va="center",
        fontsize=21, weight="bold", color='red', zorder=4)

ax.text(0.5, 0.32, r"$= \binom{4}{2} \times (0.4)^3 \times (0.6)^2 = 0.1382$",
        transform=ax.transAxes, ha="center", va="center",
        fontsize=19, color='red', zorder=4)

# Arrow from first sequence to counting box - point to top of counting box
x0, y0 = seq_boxes["SSFF•S"]
ax.annotate("Count all\nsequences",
            xy=(0.30, 0.622), xycoords=ax.transAxes,
            xytext=(0.18, 0.67), textcoords=ax.transAxes,
            arrowprops=dict(arrowstyle="->",
...
# Arrow from last sequence to probability box - point to top of probability box
x1, y1 = seq_boxes["FFSS•S"]
ax.annotate("Each has same\nprobability",
            xy=(0.70, 0.535), xycoords=ax.transAxes,
            xytext=(0.82, 0.61), textcoords=ax.transAxes,
            arrowprops=dict(arrowstyle="->",
                            connectionstyle="arc3,rad=0.2",
                            lw=2.5, color="darkgreen",
                            shrinkA=0, shrinkB=5),
            fontsize=15, color="darkgreen", weight="bold",
            ha="center", va="center", zorder=5)

# Key insight at bottom
ax.text(0.5, 0.21, "Key Insight: Last trial MUST be the r-th success!",
        transform=ax.transAxes, ha="center", va="center",
        fontsize=18, style="italic", weight="bold", color='purple', zorder=4)

ax.text(0.5, 0.165, r"So we arrange $(r-1)$ successes in the first $(k-1)$ trials, then guarantee success on trial $k$.",
        transform=ax.transAxes, ha="center", va="center",
        fontsize=16, style="italic", zorder=4)
...
ax.text(0.5, 0.095, why,
        transform=ax.transAxes, ha="center", va="center",
        fontsize=16, style="italic", wrap=True, zorder=4)

ax.text(0.5, 0.045, r"This combines: choosing positions for $(r-1)$ successes × probability of $r$ successes × probability of $(k-r)$ failures.",
        transform=ax.transAxes, ha="center", va="center",
        fontsize=15, style="italic", color='gray', zorder=4)
